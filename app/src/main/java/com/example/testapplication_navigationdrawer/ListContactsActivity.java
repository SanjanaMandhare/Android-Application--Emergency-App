...
public class ListContactsActivity extends AppCompatActivity {
    ...
    private void parseJson(String s) {
        ...
        try {
            ...
            headerList.add("Women Helpline");
            JSONArray women = place.getJSONArray("women");
            child = new ArrayList<>();
            for(int i = 0; i < fire.length(); i++) {
                child.add(women.get(i).toString());
            }
            childList.put("Women Helpline", child);

            headerList.add("Blood Bank");
            JSONArray blood = place.getJSONArray("blood");
            child = new ArrayList<>();
            for(int i = 0; i < fire.length(); i++) {
                child.add(blood.get(i).toString());
            }
            childList.put("Blood Bank", child);
